...
function toTime(date, isGregorian)
{
    if( Object.keys(date).length > 0 && ( typeof(date.year) === "number"  && typeof(date.month) === "number"  && typeof(date.day) === "number" )  && (date.year >= 0 && date.month >= 0 && date.day >= 0) )
    {
        if(!isGregorian){
            ...
            let convertedDate = calendarConverter.convertToGC(date.year, date.month + 1, date.day);
            return new Date(convertedDate.year, convertedDate.month - 1, convertedDate.day).getTime();
        }
...
function compareDates(day ,month, year , isGregorian , minDate , maxDate){
    ...
    if(!minDate.hasValue && !maxDate.hasValue){
        ...
        return isGregorian && (toTime(minDate.gregDate,isGregorian) <= toTime({year : year , month : month , day : day},isGregorian)) && (toTime(maxDate.gregDate , isGregorian) >= toTime({year : year , month : month , day : day},isGregorian))  ? 
                    true : 
                    !isGregorian && (toTime(minDate.ethDate,isGregorian) <= toTime({year : year , month : month , day : day},isGregorian)) && (toTime(maxDate.ethDate,isGregorian) >= toTime({year : year , month : month , day : day},isGregorian)) ?
                        true : false;
    } else if (minDate.hasValue){
        return isGregorian && (toTime(minDate.gregDate ,isGregorian ) <= toTime({year : year , month : month , day : day},isGregorian))  ? 
                    true :
                    !isGregorian && (toTime(minDate.ethDate ,isGregorian) <= toTime({year : year , month : month , day : day},isGregorian)) ?
                        true : false;
    } else if(maxDate.hasValue){
        return isGregorian && (toTime(maxDate.gregDate,isGregorian) >= toTime({year : year , month : month , day : day},isGregorian))  ?
                    true :
                    !isGregorian && (toTime(maxDate.ethDate,isGregorian) >= toTime({year : year , month : month , day : day},isGregorian))  ?
                        true : false;
    }
...
function compareMonths(month, year, isGregorian, minDate, maxDate){
    if(!minDate.hasValue && !maxDate.hasValue){
        ...
        return isGregorian && (toTime({...minDate.gregDate, day: 1}, isGregorian) <= toTime({year : year , month : month , day : 1}, isGregorian)) && (toTime({...maxDate.gregDate, day: 1}, isGregorian) >= toTime({year : year , month : month , day : 1}, isGregorian))  ? 
                    true : 
                    !isGregorian && (toTime({...minDate.ethDate, day: 1}, isGregorian) <= toTime({year : year , month : month , day : 1}, isGregorian)) && (toTime({...maxDate.ethDate, day: 1}, isGregorian) >= toTime({year : year , month : month , day : 1}, isGregorian)) ?
                        true : false;
    } else if (minDate.hasValue){
        return isGregorian && (toTime({...minDate.gregDate, day: 1}, isGregorian) <= toTime({year : year , month : month , day : 1}, isGregorian))  ? 
                    true :
                    !isGregorian && (toTime({...minDate.ethDate, day: 1}, isGregorian) <= toTime({year : year , month : month , day : 1}, isGregorian)) ?
                        true : false;
    } else if(maxDate.hasValue){
        return isGregorian && (toTime({...maxDate.gregDate, day: 1}, isGregorian) >= toTime({year : year , month : month , day : 1}, isGregorian))  ?
                    true :
                    !isGregorian && (toTime({...maxDate.ethDate, day: 1}, isGregorian) >= toTime({year : year , month : month , day : 1}, isGregorian))  ?
                        true : false;
    }
}
...
function calendarPicker(settings){
    ...
    const { defaultCalendar, showDatePicker, dateFormat, minDate, maxDate, isInputGregorian } = this.settings;
    ...
    convDate = this.settings["selectedDate"] ?
                    (isInputGregorian || (isInputGregorian === undefined)) ? 
                            calendarConverter.convertToEC(this.settings["selectedDate"].year, this.settings["selectedDate"].month, this.settings["selectedDate"]) : 
                            calendarConverter.convertToGC(this.settings["selectedDate"].year, this.settings["selectedDate"].month, this.settings["selectedDate"]) : {};
    ...
    let curDate;
    if(this.setting["selectedDate"]){
        if(((isInputGregorian === undefined) && initialCalendarState) || (isInputGregorian === initialCalendarState)){
    ...
    this.onDateSelected = (day) => {
        ...
        */
        ...
        this.onChangeCallback({
            "ET": {ethDateObj, ethDateStr},
            "GR": {gregDateObj: gregorianDate, gregDateStr},
            "selected": this.state.isGregorian ? "gr" : "et",
            "id": this.settings.selector
        }) 
        this.destroy();
    }
...
function pickerContainer(childComponent, pickerType, today, year , month , monthDays ,  monthName , isGregorian , showPicker, minDate, maxDate, years )
{
    ...
    return `
            ...
            >
                    <div data-isof-calendar="1"  tabIndex="-1888" id="calendarParent" class=" vstack d-flex flex-column justify-content-start f-07 "> 
                  ${
                        pickerType === "DATE" ? 
                        `    
                            <div class="d-flex justify-content-between align-items-center p-0 fs-16" id="calendarHeader">
                                <button 
                                    data-isof-calendar="1"  
                                    tabIndex="-1888" class="btn btn-outline-primary py-2 px-2"
                                    id="btn-today"
                                >
                                    ${today}
                                </button>
                  ...
                  ${
                        pickerType === "DATE" ?
                        `
                            <div data-isof-calendar="1"  tabIndex="-1888" id="monthsListController" class="f-1p3  d-flex  justify-content-between align-content-end p-0 m-0 mt-2 mb-3">
                                <span data-isof-calendar="1"  tabIndex="-1888" class="p-0 py-2 m-0 fs-6">
                                ...
                                <div data-isof-calendar="1"  tabIndex="-1888" id="monthSwitcher" class="f-09 d-flex gap-2">
                                    <button
                                        data-isof-calendar="1"  tabIndex="-1888"
                                        id="btnmonthprev"
                                        class="monthSwitcherBtn border-0 m-0 p-0 me-3"
                                        ${compareDates(monthDays[0][0].day,monthDays[0][0].month,monthDays[0][0].year, isGregorian, minDate, {}) ? "" : "disabled"}
                                    >
                                    <svg xmlns="http://www.w3.org/2000/svg" height="26" viewBox="0 -960 960 960" width="26" fill="currentcolor"><path d="M560-253.847 333.847-480 560-706.153 602.153-664l-184 184 184 184L560-253.847Z"/></svg>

                                    </button>
                                    <button
                                        data-isof-calendar="1"  tabIndex="-1888"
                                        id="btnmonthnext"
                                        class="monthSwitcherBtn border-0 m-0 p-0 me-1"
                                        ${compareDates(monthDays[5][6].day,monthDays[5][6].month,monthDays[5][6].year, isGregorian, {},maxDate) ? "" : "disabled"}
                                    >
                                    <svg xmlns="http://www.w3.org/2000/svg" height="26" viewBox="0 -960 960 960" width="26" fill="currentcolor"><path d="m517.847-480-184-184L376-706.153 602.153-480 376-253.847 333.847-296l184-184Z"/></svg>
                        ...
                        pickerType === "MONTH" ?
                        `
                        <div  data-isof-calendar="1"  tabIndex="-1888" id="monthsListController" class="f-1p3  d-flex  justify-content-between align-content-end p-0 pt-1 mb-1 mt-3">
                            <span data-isof-calendar="1"  tabIndex="-1888" class="p-0 py-2 m-0 fs-6">
                                <span data-isof-calendar="1"  tabIndex="-1888" id="activeyear" class="ms-2">
                                    ${year}
                                </span>
                            ...
                            <div data-isof-calendar="1"  tabIndex="-1888" id="monthSwitcher" class="f-09 d-flex gap-2">
                                <button
                                    ...
                                    id="btnmonthprev"
                                    class="monthSwitcherBtn border-0 m-0 p-0 me-3"
                                    ${compareMonths(isGregorian ? 11 : 12, year - 1, isGregorian, minDate, {}) ? "" : "disabled"}
                                ...
                                <button data-isof-calendar="1"  tabIndex="-1888"
                                    id="btnmonthnext"
                                    class="monthSwitcherBtn border-0 m-0 p-0 me-1"
                                    ${compareMonths(0, year + 1, isGregorian, {}, maxDate) ? "" : "disabled"}
                                >
                                <svg xmlns="http://www.w3.org/2000/svg" height="26" viewBox="0 -960 960 960" width="26" fill="currentcolor"><path d="m517.847-480-184-184L376-706.153 602.153-480 376-253.847 333.847-296l184-184Z"/></svg>
                        ...
                        :
                        `
                        <div data-isof-calendar="1"  tabIndex="-1888" id="monthsListController" class="f-08 d-flex align-content-end d-flex mb-1 pt-1">
                            <div  data-isof-calendar="1"  tabIndex="-1888" id="monthSwitcher" class="d-flex justify-content-between f-1p3 w-100" >
                                <button data-isof-calendar="1"  tabIndex="-1888"
                                    id="btnmonthprev"
                                    class="monthSwitcherBtn col-1 border-0 m-0 p-0 bi-svg" 
...
function monthPicker(months, year ,isGregorian, minDate, maxDate){
    ...
    return `
            <div class="d-flex justify-content-between flex-wrap f-1 g-1" data-isof-calendar="1"  tabIndex="-1888">${
                months.map((month,monthIdx) => {
                    return `
                        <span  data-isof-calendar="1"  tabIndex="-1888" class="col d-flex justify-content-center mt-1 fs-15">
...
function yearPicker(years, isGregorian, minDate, maxDate){
    ...
    return `
            <div data-isof-calendar="1"  tabIndex="-1888" class="f-1 d-flex align-items-between flex-wrap year-gap mt-1">
                ${
                    years.map(year => {
                        return `
                            <span data-isof-calendar="1"  tabIndex="-1888" class="my-1 col d-flex justify-content-center fs-15">
                                <button data-isof-calendar="1"  tabIndex="-1888"
...
function getEthiopicDay(month, year, currentDay){ 
   ...
   function GCtoET(year,month,day){
    var eday = day;
    var emonth = month;
    var eyear = year;
    ...
    var dateInput2 =  new CalendarConverter();//new convertToEC({"year": etDate[0], "month": etDate[1], "day": etDate[2]});

    var converter=dateInput2.convertToEC(parseInt(year),parseInt(month), parseInt(day));
    // console.log(converter,'tinoooo');
    eday = converter.day;
    emonth = converter.month;
    eyear = converter.year;

   // var result_date = eday + '-' + emonth + '-' + eyear;
    //console.log(result_date,'tinoooo');
    // $('#date').val(result_date);
    //return result_date;
    return {eday,emonth,eyear};
...
function handleDateSelection(param, id) {
    console.log('Picker Output', param);

    if (param.selected == 'et') {
        const ethDateObj = param['ET'].ethDateObj;
        const formattedDate = `${formatWithLeadingZero(ethDateObj.day)}-${formatWithLeadingZero(ethDateObj.month)}-${ethDateObj.year}`;
        document.getElementById(id).value = formattedDate;
    } else {
        const gregDateObj = param['GR'].gregDateObj;
        const formattedDate = `${formatWithLeadingZero(gregDateObj.day)}-${formatWithLeadingZero(gregDateObj.month)}-${gregDateObj.year}`;
        document.getElementById(id).value = formattedDate;
    }

    const gregDateObj = param['GR'].gregDateObj;
    const formattedDate = `${formatWithLeadingZero(gregDateObj.day)}-${formatWithLeadingZero(gregDateObj.month)}-${gregDateObj.year}`;
    document.getElementById(id + 'value').value = formattedDate;
}
function createAndInitCalendarPicker(selector, targetElementId, dateValue, onChangeCallback) {
    //console.log(dateValue, "tttttt");
    ...
    var dateInput = new calendarPicker({"defaultCalendar": "ET","selector": selector,"showDatePicker": true,"dateFormat": "dd-mm-yyyy",'onChangeCallback': (param) => handleDateSelection(param, targetElementId),selectedDate: { year, month, day }
    });
    dateInput.init();
}


function convertAndFormatDate(dateValue, targetElementId, selector, onChangeCallback) {
    console.log(dateValue, "tttttt");
    ...
    var convertedDate = GCtoET(year, month, day);
    var formattedDate = formatWithLeadingZero(convertedDate.eday) + '-' + formatWithLeadingZero(convertedDate.emonth) + '-' + convertedDate.eyear;

    $('#' + targetElementId).val(formattedDate);
    $('#' + targetElementId).text(formattedDate);
    createAndInitCalendarPicker(selector, targetElementId, formattedDate, onChangeCallback);
